...
function serializeWorkbook(parsedWorkbook, dest) {
  ...
  for (let name in parsedWorkbook) {
    ...
    // split per row to json file
    if (name.startsWith('#')) {
      sheet.forEach((content, index) => {
        let formatIndex = (index + 1).toString().padStart(4, '0');
        let rowName = name.slice(1) + formatIndex;
        parsedWorkbook[rowName] = content;
      });
      delete parsedWorkbook[name];
    }
  }
  ...
  for (let name in parsedWorkbook) {
    ...
    if (resultJson === undefined) { continue; }
