...
function parseSettings(workbook) {
  ...
  workbook.forEach(sheet => {
    ...
    if (head_row == undefined) {
      console.error(`[Error]: Cannot parse sheet: ${sheet.name}, cause head row is undfined\n`);
      return;
    }
...
function parseSheet(sheet, setting) {
  ...
  sheet.data = sheet.data.filter(obj => Object.keys(obj).length > 0);
  console.log('  * sheet:', sheet.name, 'rows:', sheet.data.length);

  if (sheet.data.length < headIndex) {
    //没有数据，不解析
    result = {}
    return;
  }
...
function parseRow(row, rowIndex, head) {
  log.log("parseRow:" + '--' + rowIndex + head + "\n");
  let result = {};
  let id;

  // check if the row is empty
  const allEmpty = row.every(item => item.length === 0);
  ...
  for (let index = 0; index < head.length; index++) {
    ...
    switch (type) {
      ...
      case DataType.OBJECT:
        ...
        break;
      case DataType.ARRAY:
        cell = toSafeArray(cell.toString());
        result[name] = parseJsonObject(cell);
        break;
  ...
  if (true === Object.values(result).every(value => value == null)) {
    return null;
  }
  return result;
}
...
/**
 * Try to convert array to valid string for JS's eval
 */
function toSafeArray(str) {
  str = str.replace(/(\r\n|\n|\r)/gm, "");
  str = str.replace("[", "");
  str = str.replace("]", "");
  ...
  result += ']';
  log.log('toSafeArray:', str, "->", result)
  return result;
}
